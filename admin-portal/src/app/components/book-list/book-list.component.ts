...
export class BookListComponent implements OnInit {
  ...
  updateSelected(checked: boolean) {
    if(checked) {
      ...
      this.removeBookList = this.bookList.slice();//deep copy
    } else {
  ...
  openDialog(book:Book) {
    ...
    dialogRef.afterClosed().subscribe(
      result => {
        console.log(result);
        if(result == "yes"){
          this.removeBookService.remove(book.id).subscribe(
            res => {
              console.log(res);
              this.getBookList();
            },
    ...
    dialogRef.afterClosed().subscribe(
      result => {
        ...
        if(result == "yes"){
          ...
          location.reload();//use getBookList may result error, because may execute this line before remove method!
        }
      }
    );
  }
