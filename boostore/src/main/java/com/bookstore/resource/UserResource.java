...
public class UserResource {
	...
	public ResponseEntity newUserPost(HttpServletRequest request, @RequestBody HashMap<String, String> mapper) throws Exception {
		...
		if(userService.findByUsername(username)!= null) {
			return new ResponseEntity("usernExists", HttpStatus.BAD_REQUEST);
		}
		...
		Role role = new Role();
		role.setRoleId(1);// 1 normal user
	...
	public User getCurrentUser(Principal principal){//Principal:find java.security package, get current loggedin user in session.
		User user = new User();
		if( null != principal){
